...
class ChatRoomControllerTest {
    ...
    class WhenRequestingToCreateNewChatRoom{
        @Test
        @WithMockUser
        @DisplayName("새로운 채탱방을 생성한다.")
        void ThenCreateNewChatRoom() throws Exception {
            ChatRoomCreateRequestDto chatRoomCreateRequestDto = new ChatRoomCreateRequestDto(1L);

            ObjectMapper mapper = new ObjectMapper();
            String requestBody = mapper.writeValueAsString(chatRoomCreateRequestDto);

            // given
            given(chatRoomService.createRoom(any(), any()))
                    .willReturn(ChatRoomCreateResponseDto.of
                            (1L,
                            1L,
                            "minnnisu",
            ...
            mockMvc.perform(
                    ...
                    .andExpect(jsonPath("$.creator").value("minnnisu"))
        ...
        @Test
        @WithMockUser
        @DisplayName("NoSuchPostError를 발생시킨다.")
        void thenThrowNoSuchPostError() throws Exception {
            ErrorCode userPermissionDeniedError = ErrorCode.NoSuchPostError;
        ...
        @Test
        @WithMockUser
        @DisplayName("AlreadyExistChatRoomError를 발생시킨다.")
        void thenThrowAlreadyExistChatRoomError() throws Exception {
            ErrorCode userPermissionDeniedError = ErrorCode.AlreadyExistChatRoomError;
